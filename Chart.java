...
public class Chart extends ApplicationFrame{
	static int pos,neg,neu,similar;
	static String title;
	static ArrayList<Labels> labels;
public Chart(String paramString,ArrayList<Labels> l){
	super(paramString);
	labels = l;
...
private static CategoryDataset createDataset(){
	...
	for(int i=0;i<labels.size();i++){
		Labels lab = labels.get(i);
		if(lab.getLabel().equals("Positive"))
			pos = pos + 1;
		if(lab.getLabel().equals("Negative"))
			neg = neg + 1;
	}
	localDefaultCategoryDataset.addValue(pos,"Positive Reviews","Positive Reviews");
	localDefaultCategoryDataset.addValue(neg,"Negative Reviews","Negative Reviews");
...
private static JFreeChart createChart(CategoryDataset paramCategoryDataset){
	JFreeChart localJFreeChart = ChartFactory.createBarChart(title,"Sentiment Type", "Count", paramCategoryDataset, PlotOrientation.VERTICAL, true, true, false);
